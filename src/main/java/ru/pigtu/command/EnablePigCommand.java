...
public class EnablePigCommand {

    public static boolean fastCrystal = true;

    public void initializeToggleCommands() {
        ClientCommandRegistrationCallback.EVENT.register((dispatcher, registryAccess) ->
                dispatcher.register(ClientCommandManager.literal("pig").executes(context -> {
                    if (fastCrystal) {
                        fastCrystal = false;
                        displayMessage("Pig disabled!");
                    } else if (!fastCrystal) {
                        fastCrystal = true;
                        displayMessage("Pig enabled");
                    }
                    return 1;
                }))
        );
